...
> 源码 [src/service/chat/](../../../src/service/chat/) ｜ 上级 [service/README.md](README.md) ｜ 相关 [./message.md](message.md)、[./websocket.md](websocket.md)、[../agent/middleware.md](../agent/middleware.md)、[db.md](../db/README.md)、[../interaction.md](../../shared/protocol/interactions.md)
...
- **协议映射**（streamMapper.ts）：`MiddlewareChunk` → 协议 `Chunk` / `Notification`（`sense_end`→`interrupt`、`sense_accept/reject`→`accept/rejected`、`question_batch_pending`→`question_batch_requested`、`message_updated` 带 replace → `replaced` 等）。ask_user_question 不再产生 `sense_started`，问题指示器直接由批次投影派生。`sense_started` / staged `sense_end` 均携带 `security`（该工具 `authorizeToolCall` 判定原样透传，见 [protocol.md §Notification](../../shared/protocol/websocket.md)），供前端渲染统一风险徽章。
...
- **chat 管理**（handler.ts + archive.ts）：`chat.create` / `list` / `get`（流式载入历史 + `canResume`）及会话归档管理。普通移除通过 `chat.archive` 归档统一关联集合；`chat.delete` 只永久清理已归档的组主会话。`chat.create` 预设路径默认**空白复用**：命中同预设 `turnCount===0` 的 root 会话即直接返回其 chatId（不新建、不 ensureChat，响应带 `reused: true` 并回显该会话持久化 runtime）；`skipBlankReuse: true` / 显式 `chatId` / `parentChatId` / 显式 brain 路径均跳过复用。
...
| [src/service/chat/handler.ts](../../../src/service/chat/handler.ts)               | `handleChatCreate` / `handleChatList` / `handleChatGet`（流式历史 + canResume）+ `registerChatManageHandlers`                                                                                                           |
| [src/service/chat/archive.ts](../../../src/service/chat/archive.ts)               | `handleChatArchive` / `handleArchiveList` / `handleChatDelete`：整组归档、归档目录与永久清理                                                                                                                            |
| [src/service/chat/lifecycleEvents.ts](../../../src/service/chat/lifecycleEvents.ts) | `broadcastChatLifecycle`：向全部窗口广播归档或删除后的目录失效通知                                                                                                                                                       |
...
`config.save` 检测到角色改名（同 `roles.<name>.id` 不同名）时，经 `migrateRoleRename` 同步迁移存量 DB 引用（`metadata.type` / `metadata.spawnTypes` / `spawn_tasks.type`），使改名对历史子 chat 关联不可见（详见 [../db.md](../db/README.md)「角色改名迁移」）。
...
> 末尾未完成撤回**只在 chat.get 恢复场景**（服务重启后末尾留未完成周期）触发。正常运行中 loop 自动续接不会留 pending。详见 [../interaction.md](../../shared/protocol/interactions.md) chat.send。
...
| `staged`                            | `createChunk("staged", rid, {type: stagedType, thinking?, content?, senseName?, arguments?, id?, msgId?, role?, createdAt?, security?}, {chatId,runId})` | 阶段完成。实时路径（`chat.send`/`resume` 执行中）`thinking_end`/`content_end` 携预分配 `msgId`+`createdAt`（`content_end` 另携 `role:"assistant"`），与 chat.get 回放同 id，供前端实时累积进 `stream.history`。`security` 仅 `sense_end` 携带（= 该工具 sense_started 的授权判定，checkpoint 从 trigger 透传） |
| `sense_end`（SenseTriggerChunk）    | `createNotification("interrupt", rid, {approvalId:id, senseName, arguments, supervisionLevel, needsApproval: level>auto, security})`                    | 感官触发（仅 smart/manual）。`security` = `authorizeToolCall` 输出原样透传（ApprovalCard 渲染危险性）                                                                                                                      |
...
`agent.resume()`（builder.ts）：末尾有 pending sense → 置 `resumePending=true`，首轮 senseMiddleware skip chat 层、重发 `sense_end`→`interrupt`（按监管等级）；全 done → `run("")` 正常 loop。续接规则与交互序列见 [../interaction.md](../../shared/protocol/interactions.md) chat.resume，agent 侧实现见 [../agent/middleware.md](../agent/middleware.md)。
...
for msg of messages:
  ...
  if parsed.senseCall: for sc: yield staged{type:"sense_end", role, senseName, arguments, id: sc.id, ...(sc.security ? {security: sc.security} : {})}
...
`canResume` 判定依据：末条 sense（pending / done 无后续 assistant）或末条 user（异常中断，resume Case2 复用末条 user 调 LLM）。详见 [db.md](../db/README.md)「状态判定」。
...
调度策略与协议影响详见 [../agent-pet.md §5.4](../../shared/architecture/agent-orchestration.md)。
...
- [@/db/chat](../../../src/db/chat.ts)：`createChat`/`listAllChats`/`getChat`/`getMessages`/`deleteChat`/`parseMessageRow`/`addMessage`/`fillApprovalResult`/`markMessagesRevoked`/`markMessageReplaced`/`updateChatMetadata`/`getChatRuntimeSelection`。详见 [db.md](../db/README.md)。
...
- [../interaction.md](../../shared/protocol/interactions.md)：chat.send / chat.resume / 审批 / 恢复的端到端交互序列与流程图。
...
- **撤回策略调整**：`revokeTrailingCycle`（agent 层）决定撤回范围；service 侧 `markMessagesRevoked` + `staged.reverse` chunk 是固定落库与通知，撤回规则变更同步 [../interaction.md](../../shared/protocol/interactions.md)。
