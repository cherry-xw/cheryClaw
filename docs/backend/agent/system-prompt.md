...
> 提示词来源信息汇总入口。实现细节见 [Agent Prompt](prompt.md)；运行时消息链见 [Agent Middleware](middleware.md)；工具定义见 [Agent Sense](sense.md)。
...
完整扫描、frontmatter 和 token 规则见 [Agent Prompt](prompt.md)。
...
详见 [Agent Sense](sense.md)、[Core Sense](../core/sense.md) 和 [Provider](provider.md)。
...
- [Agent Prompt](prompt.md)：Prompt 模块、Skill 扫描、过滤和 token 计量细节。
- [Command](command.md)：Builtin Command 注入与自动压缩。
- [Memory](../memory/README.md)：双层 Memory 存储和管理。
- [Agent Sense](sense.md)：内置 Sense 和工具 schema。
- [Core Sense](../core/sense.md)：Sense 工厂与监管等级。
- [Agent Middleware](middleware.md)：消息进入 Provider 前的执行链。
- [Provider](provider.md)：Provider 消息转换和最终 API 请求。
