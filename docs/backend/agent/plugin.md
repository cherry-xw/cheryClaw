...
与独立 skill（[skills.importUrl](../../shared/protocol/websocket.md)）对比：
...
- 日志层字段名匹配 `/(key|token|secret|password|authorization|credential|env)/i` 自动 `[REDACTED]`（[utils/logger](../utils/logger.md)），凭据字段命名为 `password` / `token` 以复用脱敏。

**凭据池**：

- 凭据池 RPC（`credentials.list/save/delete`，详见 [../protocol.md](../../shared/protocol/websocket.md)）独立于插件，可被 skills/未来 commands 复用。
...
> 凭据池 RPC（`credentials.list/save/delete`）独立于 plugins.*，详见 [../protocol.md](../../shared/protocol/websocket.md) credentials 块。
...
| [web/src/features/agent/settings/tabs/PluginsTab.vue](../../../web/src/features/agent/settings/tabs/tools/PluginsTab.vue) | settings「插件」tab 单文件视图：顶部「检查更新」工具栏 + 卡片（版本元信息默认展示 + 彩色技能 tag + 条件「拉取最新」按钮）+ 导入栏（URL → 确认拉分支 → 分支 + 文件夹名 + 导入）+ 预览 dialog |
