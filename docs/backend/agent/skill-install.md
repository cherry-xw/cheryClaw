...
**编制锁定**：Cherry Nexus 加入预设 `roles` 后，主 agent（leader）可 `spawn_role({type:"cheryNyxus", wait:true})` 派出（roster gate 自动生效，详见 [Agent 编排架构](../../shared/architecture/agent-orchestration.md)）。
