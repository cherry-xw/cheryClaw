...
> 源码 [src/utils/](../../../src/utils/) ｜ 上级 [docs/](../../) ｜ 相关 [../mock.md](../../quality/testing/mock-provider.md) ｜ [./drain.md](drain.md) ｜ [./logger.md](logger.md)
...
| [modelCatalog.ts](../../../src/utils/modelCatalog.ts) | 模型目录：精确/glob/正则匹配，提供事实、编辑器推荐和按协议分组的 thinking wire 映射 |
| [secretStore.ts](../../../src/utils/secretStore.ts) | 密钥的持久化、读取与脱敏边界，详见 [secretStore.md](secretStore.md) |
...
`BrainConfig` 关键字段（[源码](../../../src/utils/config.ts)）：`provider` / `model` / `url?` / `key?` / `thinking?` / `rpm?`（每分钟最大请求数）/ `fullUrl?`（true=URL 已含完整端点（如 `/v1/chat/completions`），provider 完全不拼接、原样访问；缺省只拼端点——**版本段（/v1）由用户填写**，各 provider 端点拼接规则见 [../agent/provider.md](../agent/provider.md)「URL 解析与端点拼接」）/ `mock?`（脚本化响应，见 [../mock.md](../../quality/testing/mock-provider.md)）/ `capabilities?`（Tool Call、图片/视频/音频输入及生成能力）。能力缺省兼容旧配置：Tool Call 开启，媒体能力关闭。
...
- `config.save` 后 service 层比对前后 `roles`（同 id 不同名）触发存量 DB 引用迁移（`migrateRoleRename`，见 [../db.md](../db/README.md)「角色改名迁移」）。
...
- **key 不参与启动校验**：`llm.brain.*.key` / `media.*.key` 缺失不阻止启动，运行期 provider 调用时若 key 为空才抛错响应前端（见 [../agent/provider.md](../agent/provider.md)）。前端 `env.list` 返回的密钥下拉**不做业务过滤**——凡 `.env` 中合法变量名（`listEnvVarNames` 按 `/^[A-Za-z_][A-Za-z0-9_]*$/` 校验）原样透出（2026-09 取消旧的后缀白名单：`API_KEY1` 这类数字尾缀命名此前会被吞掉）；运行时配置变量（`CHERY_DIR` / `*_HOST` / `*_URL` 等）也会出现在下拉，属预期行为。
...
### modelCatalog.ts — 模型目录与协议映射

`modelCatalog` 加载 `.chery/model-catalog.yaml`，并将模型数据分为三类：只读事实 `facts`、仅由设置页写入草稿的 `recommend`、运行时序列化使用的 `wire`。匹配支持精确值、glob 和结构化正则，优先级为精确 > 正则 > glob。

```ts
export function loadModelCatalog(): ModelCatalogConfig;
export function resolveModelCatalog(input): ResolvedModelCatalogEntry;
export function resolveCatalogThinkingParams(input): Record<string, unknown> | undefined;
...
会话签名密钥 `CHERY_AUTH_SESSION_SECRET` 同样持久化到该 `.env`（`ensureAuthSessionSecret`）：进程环境或 `.env` 已有则复用，否则生成 32 字节随机 hex 追加写入；删除该行后重启即轮换（见 [protocol.md](../../shared/protocol/websocket.md) 认证章节）。
...
- mock provider（依赖 config.ts 的 `MockConfig` 类型）：[../mock.md](../../quality/testing/mock-provider.md)
