...
| v1.0（增量） | 浅色可读性与 rail 交互修正（§2.1/§2.2/§4.5/§4.13，实测）：①**切换钮 ⚡ 改 switch**——标题栏切换入口由字符按钮改为 el-switch 开关（选中态实心主色轨+白钮，取代原 26×26 字符按钮的 icon 歪斜/active 不突出）；②**状态条浅色加深**——状态条/链路标签/轨迹行头文字由 `--el-text-color-secondary`（浅色 #909399 过浅）提为主文字色 `--el-text-color-primary`，链路标签非激活加边框、激活实底主色，浅色下可读；③**cluster 工具 tag 状态线弱化 + icon 增强**——底部 2px 状态条不再喧宾夺主（弱化透明度/收窄），icon 字号加大以清晰为主；④**发送按钮高度对齐**——发送按钮与左侧单行输入框等高（同 34px 单行），消除对齐错位；⑤**代码高亮浅色暖色调**——浅色模式不再用 highlight.js 默认蓝系（github.css），改暖色系覆盖（见 ui-visual-and-interaction.md §4.4） |
...
- 审批 / 提问交互以**浏览器式多页签**呈现在输入区上方的待处理面板内（v0.4.2）：面板为**独立直角卡片**（直角边框，无圆角——用户强制规则），卡内两级结构——**标签栏**（v1.2 净化：**完全透明**，无底色无背景层；唯一一条线是 bar 底部 border-bottom，位于按钮下一行；多 tab 靠最左依次排列，右端为收起按钮预留空间）+ **内容区**（亮背景，flex 自适应撑满）；**tab 按钮背景全透明**（激活态仅主色边框+主色文字区分，不加任何背景层），tab 为**直角矩形**、激活略高盖过非激活（底部 padding 差），激活不收缩、其余空间不足时非激活互相挤压、最短只剩 icon。
- **面板收起/展开（v1.1，v1.2 重做）**：**收起/展开按钮为带边框 24×24 方形按钮**（直角、字重 400，hover 主色高亮），**绝对定位挂在面板右上角、相对标签栏垂直居中**（展开态显示 ▲=收起，收起态显示 ▼=展开）；**收起态面板仅保留标签栏**（内容区高度过渡 200ms 收到 0，`prefers-reduced-motion` 直切；隐藏后 `visibility: hidden` 防止焦点落入），直接贴在输入区上方；收起态点击任意标签 = 展开并切换到该交互；新审批/提问到达**不自动展开**（保持收起，激活 tab 倒计时徽章照常提示）；收起状态按窗口 × 根会话隔离持久于 lite store（`pendingCollapsed`），切会话互不影响。
...
- **模型节点耗时匹配（v0.5.2 修正定稿）**：`projectLiteHistory` 中主/子 Agent 节点不再要求 `direction==='agent-to-user'` 才匹配 execution step（canonical rootTimeline 不投影 direction 字段，原条件恒不成立导致模型节点全回退 0 耗时、宽度全落 10px 下限），改为主/子 Agent 节点直接按 `sourceChatId` + `createdAt` 匹配 model step（窗口 5 分钟）——模型节点耗时真实生效，1s=1px 线性铺开。
...
- **固定结构**：只显示 1–2 字符的 ASCII 类型字符，字符底部叠加状态条，相邻节点使用 ASCII `|` 分隔。不显示中文类型名，也不展示参数、结果、摘要或可见耗时；完整工具名、状态和耗时只进入 `title`/`aria-label`/详情入口。
