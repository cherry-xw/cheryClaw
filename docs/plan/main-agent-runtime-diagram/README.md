...
状态：执行中。T13 投影边界、T14 结果节点树、T15 完整头部、T18 布局整理和 T16 工作台接线已完成，下一项为 T17。复杂度 4/5：沿用现有跨端事实与新版节点逻辑，重建流程拓扑、结果树绘制及动效，并保证当前卡牌模式不受影响。各小任务开发前按项目规则确认执行者，不自动沿用其他任务的执行授权，不分配子 Agent。
...
| T17 | [官网参照动效与性能](17-vueflow-motion.md)                | 4：路径动画、状态编排和可取消生命周期       | T16              | 未开始 |
...
- T16 已完成步骤详情、固定分页和对应内容回放接线；下一项为 T17，沿现有预计算路径实现官网参照动效。用户对 T16 的执行授权不自动扩展到其他任务。
...
- T07 与手册现有清单已覆盖步骤实例、严格锚点、固定回放、键盘、宽窄窗口、多窗口和当前审批隔离，无需新增重复检查项。T17 继续实现路径与节点动效，T07 最终实机确认卡牌不变及完整交互。
