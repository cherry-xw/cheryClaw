...
状态：未开始。所属[总任务](README.md)。复杂度 4/5：新版投影、完整头部、官网参照动效、卡牌冻结、恢复与真实性能。依赖 T13、T14、T15、T18、T16、T17、T19 全部完成并删除独立子计划后才进入。
...
- `workflowHeaderTemplate` / `workflowHeaderState`：模板必备路径及 Loop/重试/压缩回边、正交路由不穿步骤主体、分离端口合法性、parent 绝对位置及完整/简略头部避让；T18 约束交叉不超过 4 处、共线重叠为 0，标签锚点在对应路段且标签包围盒不遮挡节点。完成状态保留、run/轮次/尝试切换与未归属运行事实读取。定向通过不替代 A05 全量回归。
...
| M08  | 头部分组真收起与迭代展示   | 手册第 1 节 | 非活跃分组收起为标题条、活跃/用户展开保留，跨组边重路由到分组边界不悬空，Loop 轮次徽标与步骤角标正确 | 待执行 |
