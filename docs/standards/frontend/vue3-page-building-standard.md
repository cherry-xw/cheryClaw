...
5. **小部件当作自动机处理**：小型、独立、自由放置的小工具（工具提示、按钮、徽章、任务条、计数器等）按**自包含自动化卡片/自动机**处理——自身有状态流转、独立生命周期与响应式；不降级为被动模板插槽或纯展示容器。
...
- 错误信息遵守 [错误约定](../../shared/protocol/errors.md)。
...
- [ ] 小型独立小部件是否自包含状态流转（自动机），而非被动插槽/纯展示容器？
