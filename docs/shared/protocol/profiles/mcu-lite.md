...
> **文档状态：定稿（T10 终审通过，D1–D19 全部落定，用户已拍板 D13/D14）；P0 implemented；P1 实现中（in_progress，含 turn.delta 可选订阅/分页游标/折叠调优/参考固件），P2 保持 planned**。任何条目落地前必须遵循 doc-first 规范：先更新 [multi-agent-canonical-timeline.md](../../architecture/canonical-timeline.md)（权威；本文的精简投影契约已收录为其 [§3.6 精简投影（lite profile）](../../architecture/canonical-timeline.md)）与 [protocol.md](../websocket.md)（方法/字段/通知协议面已由 T11 补录）并标注 `planned/in_progress/implemented`，再写代码。
...
| `run.outcome`                                        | 原样+去重            | `{status, reasonCode, canResume, retryable, occurredAt, feedback?}`。运行终态的唯一权威；设备直接展示 feedback 的标题/说明/建议，并按 actions 提供操作。`status`、`canResume`、`retryable` 不得互相推断；同 run 后续 `done/error` 仅兼容，不能覆盖 outcome                                                                                                           | F4、用户可操作反馈 |
...
| `error`                                              | 原样                 | legacy 兼容事件。未收到同 run 的 `run.outcome` 时才按 `{message, canResume}` 降级展示；message 含前置 [tracingId] 码，设备原样显示不得截前缀。同 run 已有 outcome 时忽略，避免把循环上限 warning 覆盖成错误                                                                                                                                                          | F4、F11            |
...
5. **子 chat 的 outcome/error 折叠（T26 补充）**：`run.outcome` 与 legacy `error` 原样透传且不分根/子。设备按 chatId≠rootChatId 把子 outcome 折叠进该子任务状态行，**不得**当主回复失败展示全局错误页；同 run 已有 outcome 时忽略 legacy error。`failed` 子仍可能 `canResume=true`，设备不单独发起 resume，只呈现状态和建议。
...
- 终态以 `run.outcome` 为准：真实故障归 failed，保护性限制归 paused；`canResume` 独立判定（F4）。
...
| F4 终态语义         | `run.outcome` 单一权威；done/error 仅兼容；input.submit 立即 ack 即 Response；status、canResume、retryable 独立        |
...
| F10 doc-first       | 落地顺序条款化（A-3/D-i）：_*T5 schema → protocol.md 补录 interaction.* → lite 文档定稿 → 任何代码_*；本文即前置设计   |
...
4. _*interaction.* 未入 protocol.md 欠账_*：P0 前置步骤① 专项解决——闭环（时序约束已条款化）。
