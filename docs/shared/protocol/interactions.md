...
> [multi-agent-canonical-timeline.md](../architecture/canonical-timeline.md) 为准。

各方法的完整交互序列（C→S / S→C 实际消息流）。协议规范（传输格式、消息结构、字段定义、错误码）见 [protocol.md](websocket.md)。
...
> - **历史跨纪元完整加载**（不按 epoch_id 过滤，见 [context-epochs.md 历史连续性与兼容投影](../architecture/context-epochs.md#历史连续性与兼容投影)）→ resume 与 send 一样携带全部历史消息；若因历史数据异常构建的消息列表仍无 user 内容，chat middleware 抛 validation 错误（防御性兜底，`[tracingId] 该会话当前纪元没有可延续的用户消息，请重新发送`），error 通知 `retryable=false`，前端不显"继续"按钮
...
> 错误码全集见 [protocol.md](websocket.md)「错误处理」。
