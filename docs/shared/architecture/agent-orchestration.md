...
> 跨前后端设计文档 ｜ 前身：纯装饰桌宠（[docs/frontend/pet/](../../frontend/pet/)，commit d798ecc）｜ 关联：[protocol.md](../protocol/websocket.md)、[service/chat.md](../../backend/service/chat.md)、[db.md](../../backend/db/README.md)、[web/pet/](../../frontend/pet/)
>
...
| 数据模型                              | [PetInstance](../../../web/src/domain/pets/types.ts) 加 `chatId`/`parentChatId`/`agentType`/`isWorking`/`contextUsage`/`runtime?`；chat 表加 `parent_chat_id`；多主并存                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
- `systemPrompt` 路径相对 `.chery`（loadConfig 解析为绝对）；缺省 → 全局。per-agent prompt 数据流见 [agent/prompt.md](../../backend/agent/prompt.md)。
...
### 4.1 后端 chat 表（[db.md](../../backend/db/README.md) 扩展点）
...
### 4.2 前端 PetInstance（[types.ts](../../../web/src/domain/pets/types.ts)）
...
`StreamState`（[web/src/stores/agents/types.ts](../../../web/src/stores/agents/types.ts)）同时保存结构化 `outcome` 与 legacy `error` 投影。主要写入路径：

1. **sendMessage 捕获 done Promise**（[web/src/stores/agents/index.ts:319-346](../../../web/src/stores/agents/index.ts#L319-L346)）：解构 `{ requestId, done }`，`done.then(res => { if (!res.success) stream.error = res.error?.message; }).catch(e => { stream.error = "连接中断: " + e.message; })`。运行的 final Response 恒 success:true；业务终态由 `run.outcome` 表达（真实故障为 failed，保护性限制为 paused），`canResume` 独立驱动继续按钮；`!res.success` 仅参数错误等 RPC 失败命中。
2. **routeNotification run.outcome 分支**：按 `runId` 保存首个权威终态；error severity 才写 `stream.error`，warning/info 使用结构化反馈卡，不冒充错误。
3. **routeNotification error 分支**：仅用于旧服务端或缺失 outcome 的降级兼容；同 run 已收到 outcome 时忽略，避免覆盖权威 severity/status。
4. **resumeAgent 同样捕获 RPC/连接失败**（[web/src/stores/agents/index.ts:354](../../../web/src/stores/agents/index.ts#L354)）。

UI 反馈：[web/src/features/pets/PetSprite.vue](../../../web/src/features/pets/components/PetSprite.vue) 在 work-bubble 旁渲染 `error-bubble`（`v-if="stream?.error"`），红边浅红底，`retainUntil = Date.now() + 30000`（30s 自动隐藏）。Pet action 立即恢复 walk（不等 retain），与 done 后 20s 冻结区分。
...
子 agent 用完即转 ghost（见 §5.6），不再支持主 agent 主动销毁。`destroy_role` sense 及其历史实现 `src/agent/sense/destroy.ts`、`role_destroyed` notification 发出方已移除（`spawnBroker.emitRoleDestroyed` 保留作防御性死代码）。会话列表 ✕（`chat.delete`）仍级联删子 chat。
...
### 5.3 协议扩展（[protocol.md](../protocol/websocket.md)）
...
- **2026-07-09 废除**：spawn 重构为「一律 yield turn + 后端注入唤醒 + wakeScheduler 按策略唤主」（§5.4），结果不再由前端 RPC 回传。`subagent.result` handler、历史文件 `src/service/subagent/result.ts` 与 `spawnBroker.resolveSpawnResult` 全删；前端 `agentApi.subagentResult` 方法与旧回传分支也已删除。
...
**subagent role**：新增后端 role；历史实现曾在 `src/agent/message/adapter.ts` 扩展 Role，当前相关类型入口包括 [db/chat.ts](../../../src/db/chat.ts) MessageData、[core/middleware/types.ts](../../../src/core/middleware/types.ts) AgentMessage 与 [service/message/types.ts](../../../src/service/message/types.ts) StagedChunkData（DB role 自由 TEXT，无迁移）。Provider buildMessages 加 `subagent→user` 映射（防 OpenAI 拒未知 role）。前端 `subagent` 本就是显示 role（MessageBubble 零改），accumulateStaged 加 role:subagent 分支承接注入消息。
...
- **前端 hover 面板**：[MessageBubble.vue](../../../web/src/features/agent/chat/MessageBubble.vue) 从 `item.runtime` 取 brain/senseGroup/mcpServers（不再从 pet.runtime 查）。user 不弹面板。
...
> **现行实现说明：** 下方早期的 `mergedChildChatId` / `messages.metadata.childChatId` 透传方案已废弃。展示合并现在完全在前端 [historyMerge.ts](../../../web/src/stores/agents/data/historyMerge.ts) 完成：仅以每个子 chat 的最终 assistant 回复为候选，与正文完全相同且时间不晚于它的主 chat `role` 回传一对一匹配；并列或缺失时保留两条。原始 DB 记录与 staged 协议不携带展示配对字段。主 group 历史递归纳入全部后代，子 direct 历史只显示自身。合并项保留子回复的内容、thinking、runtime 和工具调用，使用回传时间，并显示“子 pet → 实际父 pet”混合头像。
...
前端累积：[streamAccumulator.ts:92-98](../../../web/src/stores/agents/data/streamAccumulator.ts#L92-L98) 处理 `content_end role=role/subagent` 时写入 `mergedChildChatId: d.childChatId`。

**合并算法**（[HistoryDrawer.vue:49-57](../../../web/src/features/agent/drawer/HistoryDrawer.vue#L49-L57)）：
...
**新样式「子发送消息给主」**（[MessageBubble.vue](../../../web/src/features/agent/chat/MessageBubble.vue)）：
...
发言者大头像现按 `callerIsMaster` 区分多级 spawn，与 `role` 分支徽章写法对称（[MessageAvatar.vue](../../../web/src/features/agent/chat/MessageAvatar.vue)）：
...
### 6.3 pet 模块改造清单（[web/pet/](../../frontend/pet/) 同步）

**删除**：

- [invokeTool](../../../web/src/features/pets/composables/usePetWorld.ts) 装饰分支（pet/feed/sleep/punch/dismiss/summon）
...
| 阶段    | 功能点                                                                                                                                                                  | 状态 | 验证                                                |
| ------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---- | --------------------------------------------------- |
| **CP0** | 本文档 + 更新 [README.md](../../README.md)/[web/pet/](../../frontend/pet/) 索引                                                                                                      | ✅   | 文档评审                                            |
...
> CP0-CP7 已全部落地。CP7 的 token 估算为字符数近似（`Math.ceil(text.length/4)`，[src/utils/token.ts](../../../src/utils/token.ts)），后续接 tokenizer 替换 `estimateTokens` 实现即可；compact RPC 仅 UI 预留（PetToolbar 按钮 + PetStage TODO），后端未实现。前端组件层细节见 [web/pet/agent-integration.md](../../frontend/pet/agent-integration.md)。
