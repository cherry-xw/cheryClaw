...
运行流程中的压缩阶段与配置纪元并列，冻结资源摘要及实际消息恢复口径见[workflow 契约](../protocol/workflow.md)；不得把纪元切换当作压缩裁剪。
...
- 配置修订的 fingerprint 只覆盖**语义面**——影响系统提示词、工具契约或角色编制的字段与资源；**连接面**变更（url/key/超时等）仅热更新运行配置，不产生新修订、不切纪元。语义变化后系统在安全边界激活新修订，并让后续执行进入新纪元（仅快照与提示词切换）。保存成功与激活成功是两个状态，前者不会改写正在执行的节点树。
...
2. 有效手工修改生成 `manual` 候选并进入与结构化保存相同的统一生效协调器：操作和运行边界可局部采用，节点树与资源边界等待受影响任务安全结束，进程级影响登记为重启待办。
...
5. 冷启动时配置无法加载，Guardian 保全坏文件为 `rejected-startup-*.yaml`，从最近 `config-*.yaml` 恢复后启动维护 worker。修复保存成功后清除维护状态；仅当仍有进程级影响时才在安全边界重启。
...
`patch` 与 `rollback` 必须携带 get 返回的全配置 `baseRevision`。服务端基于当前磁盘状态构造候选，完整验证后写盘并登记候选修订；统一协调器按影响资源选择操作、运行、资源、节点树或重启边界。节点树变化等待受影响运行排空，进程级变化保留为明确的重启待办。

## 保存、生效与恢复

配置保存成功只保证候选已持久化和可审计。协调器分别记录已保存版本与已应用版本：未受影响或已替换的资源为 `applied`，等待节点树/资源安全边界的资源为 `pending`，准备或替换失败的资源为 `failed`。失败时保留最后可用运行态，不把磁盘候选静默改回旧值。

维护模式的修复路径仍可保存设置，但用户必须查看生效状态：保存后的资源失败需要修正后再次提交，进程级项需在 blocker 排空后重启。重启仅恢复可恢复的会话状态为 paused；不可序列化的 generator、正在运行的外部进程和旧 MCP client 不保证透明续接。
