...
> 上级 [docs/frontend/README.md](README.md) ｜ 相关 [electron.md](electron.md)、[deployment.md](deployment.md)
...
**`.env` 用法**：首次启动填写 `LLM_API_KEY`，再在设置页完成 `default` 大脑的地址和模型。`config.yaml` 以 `$LLM_API_KEY` 引用真实值，运行时由 [src/utils/config.ts](../../src/utils/config.ts) 替换（[优先级：OS env > `.env`](../backend/utils/README.md)）。
