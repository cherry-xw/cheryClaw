...
本文描述当前前端已经落地的 owner、迁移状态和后续工作。强制目录与依赖规则以 [Web 前端架构与目录规范](../../standards/frontend/web-frontend-architecture.md) 为唯一来源；本文不重复定义另一套规范。
...
公开端口不得暴露 `data/**`、reducer 内部 helper 或协议游标。端口的存在不代表可以无限制暴露整个 store；迁移期 backend/transport facade 的删除条件记录在 `docs/shared/architecture/issues.md`，后续应持续收窄为面向用例的接口。
...
当前目录角色、feature 子切片、store 内部 `model/read-model/bindings` 结构及依赖矩阵不在本文重复维护，统一见 [目录规范](../../standards/frontend/web-frontend-architecture.md)。
