...
> 源码 [PetSprite.vue](../../../web/src/features/pets/components/PetSprite.vue) ｜ 上级 [README.md](README.md) ｜ 动画 variant 见 [motion.md](motion.md) ｜ 样式见 [style.md](style.md)
...
- **工具细分**：tool-batch 仍按首个工具名归类，但矩阵扩充为 command/read/write/search/skill/spawn/child/question/media/todo/memory/config/navigate/role/web/data/git/time/notify/generic 共 20 类；先匹配明确内建工具名，再按有序关键词匹配，未知工具使用 `LuWrench`。
- **颜色双通道（危险程度 + 节点类型）**：Signal 使用独立的 29 类深浅主题色；危险程度覆盖优先于类型色——error 红（`stateError`）> paused 琥珀（`statePaused`）> revoked 灰（`stateRevoked`），running/detailActive 在类型色上提亮描边。
- **标签**：Signal 模式**不创建任何 Text 标签**（`rebuildLabels` 的 signal 分支为空转），`signalLabelBudget`/`SIGNAL_LABEL_*`/`protocolCode`/`summary` 投影链整体摘除；可访问性文本由 HTML `aria-label` 承载不受影响。Classic 分支标签系统不动。
- **连线（2026-09-02 三轮重构）**：水平边从节点左右端口出入；同 lane 使用直线，跨 lane 使用 `H-Q-V-Q-H` 的 8px 圆角正交线。普通分叉按来源节点共享出线干线，return 按目标节点共享汇合干线，转折点绘制小型接点；列间净距 72px、lane 间距 88px。静态线与 7 段嵌套流光消费同一份采样几何，脉冲可连续通过圆角。
...
- **渲染分辨率（2026-09-02 调整）**：GPU 画布位图分辨率 `graphDpr` 与标签纹理分辨率 `graphLabelResolution` 上限提升为 high=2 / balanced=1.5（low=1 不变）；`rendererResolution` 仍取 `min(devicePixelRatio, 上限)`，因此只影响高分屏（Windows 125%/150% 缩放等），消除画布位图低于设备像素导致的节点边缘整体发虚。
...
**交互（长按拖拽 + 短按抚摸）**：`pointerdown` 启 300ms 定时器（`LONG_PRESS_MS`）+ 记录落点；**长按超时或移动超阈值（`DRAG_THRESHOLD_PX=5`）**任一触发 → `startDrag`（`setPointerCapture` + 进入 `dragging`）；**短按（<300ms 且未超阈值）松开** → 取消定时器，不拖拽，让 `click` 触发 `clickPet` 抚摸。拖拽结束的 `pointerup` 紧随触发 `click` → `suppressClick` 标志抑制，避免拖拽完又抚摸。长按等待中离开 `.pet`（`pointerleave`）取消定时器。`onPointerMove` 在等待中检阈值，进入拖拽后透传 `drag`。（实现下沉 [usePetDrag.ts](../../../web/src/features/pets/composables/usePetDrag.ts)：常量 `LONG_PRESS_MS`/`DRAG_THRESHOLD_PX` + 私有态 + handler + `petHover` + `onBeforeUnmount` 清 `longPressTimer` 集中；行为不变。）
...
> 源码派生 [useStreamBubble.ts](../../../web/src/features/pets/composables/useStreamBubble.ts) `isBusy` ｜ 渲染 [PetSprite.vue](../../../web/src/features/pets/components/PetSprite.vue) 模板 + `.busy-indicator` 样式
