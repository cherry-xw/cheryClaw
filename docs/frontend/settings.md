...
- 全局 Tab 保留单屏散卡画布和原有长按拿起、拖动、置顶交互；卡片允许局部遮挡，但初始化不会全部堆在同一点。初始位置在分散的落点附近随机偏移，并按实际卡片尺寸约束在画布范围内；普通编辑不重新随机，拖动可自由越过和覆盖其他卡片，松手不自动避让。画布不改为上下滚动列表；卡内控件按卡片可用宽度换行，文字与数值完整展示，不产生水平滚动条；小窗口内过高的卡片仅纵向滚动自身内容。
