...
本文维护流程图的展示与交互规则，人工验收结果由活动计划记录。跨端字段与状态口径见[workflow 契约](../shared/protocol/workflow.md)，后端写者与恢复边界见[观察实现](../backend/service/workflow.md)。

## 1. 目标与非目标

工作台提供基础运行流程图，回答“主 Agent 由哪些主要环节组成，现在运行到哪里，正在等待什么”。它与消息节点树互补：节点树保存具体内容和完整历史，流程图只表达结构、进度与简要结果。
...
| `result` | 本轮结果 | 固定骨架；本轮退出后更新 | 完成/暂停/取消/失败；等待返回不能显示完成 | 终点图标、标准节点；状态决定描边与状态图标 |
...
- 普通工具仅展示当前批次；记忆和技能只展示摘要统计。不提供逐条资源正文列表。
...
以下是布局计算采用的尺寸预算，不代替人工实机验证。`H` 为扣除工作台标题、已展开输入区及必要安全间距后的内容高度：
...
232px 的下限用于容纳紧凑控制、基础路径和三个工具行。条件节点使用主路径邻接轨道，超过图形净高的部分在图内滚动，不能持续挤占消息树。
...
这是流程开启时三栏模式的可读性下限。卡片绝对定位与树的百分比宽度由宿主局部样式调整为同一组轨道尺寸，避免先缩整个树组件，再在组件内按 50% 二次缩窄卡片。
...
`web/src/features/agent/workbench/runtime-diagram/RuntimeDiagram.vue` 以 Vue 节点、SVG 连线和原生滚动呈现流程；既有 Pixi 消息树不共享相机。`model.ts` 维护确定性回放与空间计算，`useWorkflowController.ts` 管理租约、分页和计时器，`useWorkflowLayout.ts` 只测量宿主与输入区尺寸。`useWorkbenchWorkflow.ts` 接入工作台开关和生命周期。功能只通过 `application/backend/public.ts` 暴露的 workflowApi 调用只读 RPC。

`WorkbenchOfflineMask.vue` 独立承载断连提示与重连入口；工作台装配保持单文件预算。播放前完整加载所选阶段；加载失败、循环游标或历史变化必须重新加载，不把缺页当播放结束。窗口最小化、文档进入后台或断线时暂停播放，卸载清理计时器与租约。

自动验证：`pnpm type-check:all`、`pnpm web:build`、`pnpm test:web`；新增回放与竞态测试位于 `web/test/agent/`。真实窗口、主题、滚动和交互效果由用户验证，构建通过不代表视觉验收通过。
