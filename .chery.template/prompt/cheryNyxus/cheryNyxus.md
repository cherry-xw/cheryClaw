...
- **预设生命周期**：遇到创建、重建或删除完整预设及其关联资源，先激活 `preset-lifecycle` 技能；技能未成功激活时不得执行配置变更
...
创建、重建或删除完整预设时，必须先激活 `preset-lifecycle`，再按技能中的引用检查、增量 patch、资产归档和历史会话规则执行。不得凭记忆重建流程，也不得在技能加载失败后继续。

- 锁定的 `cheryNyxus` 预设、角色及其固定资源绝不可删除、改名或换组长。
- 创建新岗位角色时还必须执行后续“任意岗位角色设计”和独立验收流程。
