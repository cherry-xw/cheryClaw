...
- **文档影响先判定**：代码修改前先判断是否改变契约、边界、稳定入口、关键流程、扩展方式或验证入口；有影响时先更新权威文档，无影响时不制造文档改动。详细规则以根目录 [AGENTS.md](../AGENTS.md) 为入口。
- **规范强制**：项目级硬约束由根目录 [AGENTS.md](../AGENTS.md) 直接提供；详细标准按作用域归档在 [docs/standards/](../docs/standards/README.md)。不要求写入任何已读凭证或本地记录。
