...
export function useExecutionNodePopoverController(
  props: ExecutionNodePopoverControllerProps,
  emit: ControllerEmit<ExecutionNodePopoverControllerEmits>,
) {
  ...
  const readFileContent = computed(() =>
    (selectedCall.value?.result ?? '')
      .replace(/\[(compressed|truncated):\s*\w+\]\s*$/, '')
      .trimEnd(),
  )
  ...
  const { html: renderedActualDescription } = useRenderedMarkdown(actualDescription, {
    mode: 'full',
  })
  ...
  const nodeTitle = computed(() => {
    if (foldPosition.value)
      return `过程组 · ${foldPosition.value.index}/${foldPosition.value.total}`
  ...
  function toolLabel(name: string): string {
    return (
      agents.senseTools.find((tool) => tool.name === name)?.label?.trim() || toSenseNameZh(name)
    )
  }
