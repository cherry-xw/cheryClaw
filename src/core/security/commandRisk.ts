  ...
  const visit = (node: unknown): void => {
    ...
    if (value.type === 'Command') {
      ...
      commands.push({
        name: word?.text,
        text:
          typeof pos === 'number' && typeof end === 'number'
...
function powershellExecutable(): { command: string; args: string[] } | undefined {
  ...
  const pwsh = spawnSync(
    'pwsh',
    ['-NoLogo', '-NoProfile', '-NonInteractive', '-Command', '$null'],
    {
      stdio: 'ignore',
      timeout: 3000,
      windowsHide: true,
...
function powershellCommands(source: string): { commands: ParsedCommand[]; errors: string[] } {
  ...
  if (!jsonLine)
    return { commands: [], errors: [result.stderr.trim() || 'PowerShell parser returned no AST'] }
  try {
    const parsed = JSON.parse(jsonLine) as {
      commands?: ParsedCommand | ParsedCommand[]
      errors?: string[]
    }
    return {
...
export function assessCommandRisk(
  ...
  for (const message of parsed.errors) {
    add(
      { code: 'shell.parse-error', category: 'unknown', severity: 'unknown', message },
      'danger-full-access',
    )
  }
  for (const entry of parsed.commands) {
    ...
    if (entry.dynamic || !name) {
      add(
        {
          ...base,
          code: 'shell.dynamic-command',
          category: 'dynamic-code',
          severity: 'unknown',
          message: '命令名在运行时动态生成，无法证明安全',
        },
        'danger-full-access',
      )
      continue
    ...
    if (DYNAMIC.has(name))
      add(
        {
          ...base,
          code: 'shell.dynamic-code',
          category: 'dynamic-code',
          severity: 'high',
          message: `${entry.name} 会动态解释或编译代码`,
        },
        'danger-full-access',
      )
    else if (SYSTEM.has(name))
      add(
        {
          ...base,
          code: 'shell.system-change',
          category: 'system',
          severity: 'high',
          message: `${entry.name} 会修改系统、服务或进程状态`,
        },
        'danger-full-access',
      )
      ...
      add(
        {
          ...
          severity: 'high',
          message: `${entry.name} 可能删除或破坏数据`,
        },
        'workspace-write',
      )
    else if (NETWORK.has(name))
      add(
        {
          ...base,
          code: 'shell.network',
          category: 'network',
          severity: 'high',
          message: `${entry.name} 会访问网络或传输数据`,
        },
        'workspace-write',
      )
    else if (FILE_MUTATORS.has(name) || WORKSPACE_DEV.has(name))
      add(
        {
          ...base,
          code: 'shell.workspace-write',
          category: 'filesystem',
          severity: 'medium',
          message: `${entry.name} 可能修改工作区`,
        },
        'workspace-write',
      )
      ...
      add(
        {
          ...base,
          code: 'shell.unknown-command',
          category: 'unknown',
          severity: 'unknown',
          message: `未知可执行文件 ${entry.name}，无法证明安全`,
        },
        'danger-full-access',
      )
  }
  if (/\b(?:sudo|runas)\b/i.test(command))
    add(
      {
        code: 'shell.privilege',
        category: 'privilege',
        severity: 'high',
        message: '命令请求提升权限',
        fragment: command,
  ...
  if (/\b(?:-encodedcommand|frombase64string)\b/i.test(command))
    add(
      {
        code: 'shell.obfuscation',
  ...
  if (/\b(?:\.env|id_rsa|id_ed25519|credentials|private[_-]?key)\b/i.test(command))
    add(
      {
        code: 'shell.credential',
        category: 'credential',
        severity: 'high',
        message: '命令可能访问凭据或私钥',
        fragment: command,
  ...
  if (
    /\b(?:curl|wget|invoke-webrequest|invoke-restmethod)\b[\s\S]*[|;]\s*(?:sh|bash|iex|invoke-expression)\b/i.test(
      command,
    )
  )
    add(
      {
        code: 'shell.download-execute',
        category: 'dynamic-code',
        severity: 'high',
        message: '命令下载远程内容后直接执行',
        fragment: command,
      },
      'danger-full-access',
    )
  if (PACKAGE_INSTALL_RE.test(command))
    add(
      {
        code: 'shell.package-install',
        category: 'network',
        severity: 'high',
        message: '依赖安装会访问网络并可能执行安装脚本',
        fragment: command,
      },
      'workspace-write',
    )
  if (
    /\bgit\s+(?:add|am|apply|checkout|cherry-pick|clean|commit|merge|mv|pull|rebase|reset|restore|rm|stash|switch)\b/i.test(
      command,
    )
  ) {
    add(
      {
        code: 'shell.git-write',
        category: 'filesystem',
        severity: 'medium',
        message: 'Git 子命令可能修改工作区或仓库状态',
        fragment: command,
      },
      'workspace-write',
    )
  ...
  if (parsed.commands.length === 0 && parsed.errors.length === 0)
    add(
      {
        code: 'shell.empty-ast',
        category: 'unknown',
        severity: 'unknown',
        message: '没有解析到可验证的命令节点',
      },
      'danger-full-access',
    )

  const decision = level === 'low' || level === 'medium' ? 'allow' : 'approval-required'
