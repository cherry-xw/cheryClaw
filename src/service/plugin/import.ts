...
import {
  Method,
  type PluginsPreImportUrlRequestData,
  type PluginsPreImportUrlResponseData,
  type PluginsImportUrlRequestData,
  type PluginsImportUrlResponseData,
  type PluginsCommitRequestData,
  type PluginsCommitResponseData,
  type PluginsCheckUpdateRequestData,
  type PluginsCheckUpdateResponseData,
  type PluginsCheckAllUpdatesRequestData,
  type PluginsCheckAllUpdatesResponseData,
  type PluginsCheckAllUpdatesFailure,
  type PluginsUpdateRequestData,
  type PluginsUpdateResponseData,
  type PluginsUninstallRequestData,
  type PluginsUninstallResponseData,
...
function readPluginStaging(stagingId: string): PluginStagingMeta {
  ...
  if (!existsSync(p))
    throw new Error(`暂存 plugin-manifest 不存在（stagingId=${stagingId}），请重新导入`)
...
export async function handlePluginsUpdate(
  ...
  if (m.cloneUrl && !m.branch)
    throw new Error(`插件 "${name}" manifest 缺 branch，无法更新（请卸载后重新导入）`)
  ...
  try {
    ...
    if ((err as { needsAuth?: boolean }).needsAuth) {
      throw new Error(
        'Git 鉴权失败：该仓库需凭据，更新暂不支持内联凭据，请用系统 git 凭据助手或卸载后重新导入',
      )
    }
    throw err
  }
