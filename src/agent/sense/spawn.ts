...
import { sense, type Sense, type SenseResult, type SenseSharedData } from '@/core/sense'
...
// - 「可见即可选」：per-chat 工具定义按 execute roster 裁剪（buildSpawnRoleSense），
//   空配置兜底 z.string()（z.enum([]) 构造抛错）。
...
const SPAWN_DESC_PREFIX = `派发角色执行子任务（主 agent 一律本轮结束停等，子群并行跑；唤醒策略控制子完成后何时唤主）。
可用角色类型（每类型能力 = brain + senseGroup）：
`
const SPAWN_DESC_SUFFIX = `
必填参数：
- type: 角色类型名（上方枚举的可用键）
- prompt: 交付角色的任务描述
 ...
 * - self-spawn 禁止：恒过滤掉当前 chat 自身角色 type（主=preset.leader，子=metadata.type，见 getChatType）。
 * 单一来源：工具 type 枚举（buildSpawnRoleSense）与执行期 gate（spawnHandler）都走它，「可见即可选」+「不可自派」双端一致。
 */
export function resolveSpawnRoster(chatId: string): string[] {
  const presetName = getChatPreset(chatId)
  const raw = presetName
 ...
 *      （immediate/策略满足）。不在后端 sense 内部跑子 agent（规避 sense 无法 trigger chat.send、
 *      跨连接 busy 锁两大风险）。
 */
function buildSpawnRoleSenseFromRoster(roster: string[]): Sense<z.ZodType> {
  const schema = roster.length > 0 ? z.enum(roster as [string, ...string[]]) : z.string()
  return sense(
    'spawn_role',
    buildSpawnDescription(roster),
    z.object({
      type: schema.describe('角色类型名（上方枚举的可用键）'),
...
/**
 * per-chat 构建 roster 感知的 spawn_role Sense（runtimeResolver.resolveSense 用）。
 * roster = resolveSpawnRoster(chatId)：preset 编制 + self-spawn 排除 → 工具只暴露本 chat 可派发角色。
 */
export function buildSpawnRoleSense(chatId: string): Sense<z.ZodType> {
  return buildSpawnRoleSenseFromRoster(resolveSpawnRoster(chatId))
}
...
async function spawnHandler(
  input: { type: string; prompt: string; wake: 'immediate' | 'deferred' | 'barrier' },
